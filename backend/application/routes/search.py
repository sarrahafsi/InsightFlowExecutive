...
async def priority_inbox(since_days: int = 7, limit: int = 10, source: str = ""):
    ...
    for item in items:
        ...
        bs = m.get("burnout_score") or 0
        score += int(bs * 4)
        # Fraîcheur — moins c'est récent, plus c'est vieux, moins prioritaire
...
@router.get("/authors")
async def list_authors(since_days: int = 30):
    """Liste tous les auteurs avec leurs stats NLP agrégées."""
...
async def author_detail(author_name: str, since_days: int = 90):
    """Profil détaillé d'un auteur avec historique horloge + business trend."""
    since = datetime.utcnow() - timedelta(days=since_days)
    items = [i for i in item_store.all(since=since, limit=5000) if i.author == author_name]
...
@router.get("/compare")
async def compare_periods(period_a: int = 7, period_b: int = 14):
    """Compare deux périodes glissantes côte à côte."""
    from data.analytics.engine import compute_overview
    now = datetime.utcnow()

    stats_a = compute_overview(item_store, since_days=period_a)
    # Pour la période B : items entre [now-period_b, now-period_a]
    since_b = now - timedelta(days=period_b)
    until_b = now - timedelta(days=period_a)
    items_b = [i for i in item_store.all(since=since_b, limit=5000) if i.timestamp <= until_b]
    ...
    period_b_label = f"J-{period_b} → J-{period_a}"
    period_a_label = f"J-{period_a} → aujourd'hui"

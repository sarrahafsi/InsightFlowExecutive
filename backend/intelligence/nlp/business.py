...
_CALENDAR_PATTERNS = (
    ...
    "has invited you", "invite accepted", "invite declined",
    "updated invitation", "new event", "reminder:", "event reminder",
    "you have been removed", "meeting request",
)

def _is_calendar_notification(item: EnrichedItem) -> bool:
    """Returns True if the message looks like a calendar/automated notification."""
    text = (item.title + " " + item.content).lower()
    return any(pattern in text for pattern in _CALENDAR_PATTERNS)
...
class BusinessClassifier(BaseProcessor):
    ...
    def process(self, item: EnrichedItem) -> EnrichedItem:
        # Calendar notifications → always Neutral Update, never send to LLM
        if _is_calendar_notification(item):
            item.business_label      = "Neutral Update"
            item.business_confidence = 0.95
            item.business_reason     = "Calendar/automated notification — no business risk."
            return item
